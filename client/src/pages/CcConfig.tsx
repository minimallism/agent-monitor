/**
 * @file CcConfig.tsx
 * @description Claude Code configuration explorer. Surfaces every skill,
 * subagent, slash command, hook, settings file, memory file, and keybinding
 * Claude Code knows about. Read access for all surfaces; create / edit / delete for the
...
function MemoryPanel({
  ...
  const { primary, autoFiltered, groups, missingScopes } = useMemo(() => {
    ...
    // Search applies to the whole tab — match the memory cards on their
  ...
  // The "create missing memory file" prompts only make sense when not filtering.
  const showMissing = !q;

  return (
    <div className="space-y-3">
      {/* Primary memory (user + project) — editable */}
      {primary.map((m) => (
